...
parse_ccc_threshold <- function(threshold) {
  if (is.numeric(threshold)) {
    return(as.numeric(threshold))
  }
  if (grepl("^CCC\\.[0-9]+$", threshold)) {
    return(as.numeric(sub("^CCC\\.", "0.", threshold)))
  }
  if (grepl("^CCC\\.[0-9]+\\.[0-9]+$", threshold)) {
    return(as.numeric(sub("^CCC\\.", "", threshold)))
  }
  return(NA_real_)
}

apply_validation_threshold <- function(df, threshold, value_col = "CCC") {
  ccc_threshold <- parse_ccc_threshold(threshold)

  if (identical(threshold, "rf_model.1")) {
    print("Using the threshold for rf_model = 1")
    return(df %>% filter(predicted == "1"))
  }

  if (identical(threshold, "rf_model.all")) {
    print("Using no threshold for rf_model")
    return(df)
  }

  if (!is.na(ccc_threshold)) {
    print(paste("Using the threshold for CCC >=", ccc_threshold))
    return(df %>% filter(.data[[value_col]] >= ccc_threshold))
  }

  stop(paste("Unsupported threshold:", threshold))
}
...
validation.canopy <- function( threshold){
  ...
  for( site in site.list){
    ...
    # Harmonize the data: #####
    df_canopy <- df %>%
      left_join(canopy.sub, by = c("dLevelsAminusB", "gas", "Approach"), relationship = "many-to-many") %>%
      mutate(time.rounded = timeEndA.local %>% round_date(unit = "30 minutes")) %>%
      apply_validation_threshold(threshold = threshold) %>%
    ...
    canopy.sub <- SITES_One2One_canopy_model %>% select( Site, dLevelsAminusB, gas, Approach, rf_model ) %>% filter( Site == site)
      ...
      df_canopy <- df %>%
        ...
        reframe(.by = c(dLevelsAminusB, gas, Approach, time.rounded),
                EC_mean = mean(EC_mean, na.rm = T),
                FG_mean = mean(FG_mean, na.rm = T)) %>%
        mutate(levels = paste(Approach, dLevelsAminusB, sep = "-"))
...
Compare.SamplingHeightPairs_AA <- function( threshold){
  ...
  for( site in site.list){
    ...
    df <-SITE_DATA_FILTERED_CCC[[site]] %>% filter(gas != "CH4") %>% 
      mutate(month = format(timeEndA.local,'%m') %>% as.numeric,
             ...
             hour = format(timeEndA.local,'%H'),
             count= case_when( is.na(FG_mean) == FALSE ~ 1,
                               TRUE ~ 0)) %>% distinct %>% filter( Canopy_L1 == "AA")
    
    
    canopy.sub <- SITES_One2One_canopy_model %>% filter( Canopy_L1 == "AA") %>%  select( Site, dLevelsAminusB, gas, Approach, rf_model) %>% filter( Site == site)
...
validation.approach_V1 <- function(index.df){
  ...
  for( site in site.list[-c(31, 42)]){
    ...
    canopy.sub <- index.df %>% select( Site, dLevelsAminusB, gas, Approach, Good.CCC.EC ) %>% filter( Site == site)
...
validation.approach_V1_AA <- function(index.df){
  ...
  for( site in site.list[-c(31, 42)]){
    ...
    df_canopy <- df %>% left_join(canopy.sub , by=c("dLevelsAminusB", "gas", "Approach" ), relationship = "many-to-many") %>% mutate( time.rounded = timeEndA.local %>% round_date( unit = "30 minutes") ) %>% 
      filter( Good.CCC.EC  == '1' , Canopy_L1 == "AA") %>%
      reframe( .by= c( gas, Approach, time.rounded), 
               EC_mean = mean(EC_mean, na.rm=T), 
               FG_mean = mean(FG_mean, na.rm=T)) 
...
validation.canopy.l1_V1 <- function(index.df){
...
validation.SamplingHeightPairs <- function( index.df){
  ...
  for( site in site.list) {
    ...
    canopy.sub <- index.df %>% select( Site, dLevelsAminusB, gas, Approach, Good.CCC.EC.FG, Good.CCC.GF) %>% filter( Site == site)
...
validation.SamplingHeightPairs_V1 <- function( index.df){
...
validation_V1 <- function( index.df){
