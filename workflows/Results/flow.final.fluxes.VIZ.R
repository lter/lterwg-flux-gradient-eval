...
for( site in site.list){
  ...
  MBR_9min_FILTER_CCC <- SITES_One2One_sub %>% filter( Approach == "MBR") %>% full_join( MBR_9min_FILTER , by = c('dLevelsAminusB'))  %>% filter(Good.CCC == 1 , Canopy_L1 != "WW", cross_grad_flag == 0) %>% 
  ...
  WP_9min_FILTER_CCC <- SITES_One2One_sub %>% filter( Approach == "WP") %>% full_join( WP_9min_FILTER , by = c('dLevelsAminusB'))  %>% filter(Good.CCC == 1 , Canopy_L1 != "WW", cross_grad_flag == 0) %>% 
    select( timeEndA.local,FG_mean ,Good.CCC , Approach, Canopy_L1, gas, TowerPosition_A, TowerPosition_B, FC_turb_interp, cross_grad_flag)
...
respiration.days <- SITE_DIEL_FINAL_Daily_C_same %>% filter(!is.na(FG.gC) & !is.na(EC.gC)) %>% filter(EC.gC >= 0, gas== "CO2")  %>% full_join(canopy.site ) %>% mutate(Site = fct_explicit_na(Site, na_level = "(Missing)"))
...
SITE_DIEL_FINAL_Daily_C_same_adj_attr <- SITE_DIEL_FINAL_Daily_C_same  %>% full_join(metadata, by = "Site")

library(mgcv)
SITE_DIEL_FINAL_Daily_C_same_adj_attr %>% names

gam.correction <-  gam( EC.gC ~ FG.gC + s(canopyHeight_m,k=4) + s(TopRugosity,k=4)  + s(LAI.sd,k=3) + s(DistZaxsGrndOfst, k=3) + IGBP , data = SITE_DIEL_FINAL_Daily_C_same_adj_attr)

gam.correction %>% summary
...
nls_model <- nls(EC.gC ~ FG.gC + a * exp(b * canopyHeight_m) +  c*RelativeDist_B + d*LAI.sd , 
                            data = SITE_DIEL_FINAL_Daily_C_same_adj_attr %>% filter( EC.gC <0),
...
SITE_DIEL_FINAL_Daily_C_same_adj_attr$nls.correction <- predict( nls_model ,SITE_DIEL_FINAL_Daily_C_same_adj_attr)

SITE_DIEL_FINAL_Daily_C_same_adj_attr %>% filter( FG.gC < 50) %>% 
  ggplot(aes(x = nls.correction, y= EC.gC, col=canopyHeight_m)) + 
  geom_abline(intercept = 0, slope = 1, col="grey") +
  geom_point( size = 1, alpha=0.5) + 
  stat_poly_line(col="black") +
  stat_poly_eq() + theme_bw() +
  theme(legend.title = element_blank())+
  scale_color_gradient2(midpoint=10, low="goldenrod", mid="darkgreen", high="springgreen", name="Canopy Height (m)" ) +theme(
    strip.text.x = element_text(size = 10, color = "white"),
    strip.background = element_rect(fill = "black")) + xlim(-30, 30) + ylim(-30,30) +
  facet_wrap( ~IGBP)


forest.sub <- SITE_DIEL_FINAL_Daily_C_same_adj_attr %>% filter( FG.gC < 50, IGBP == "DBF" |IGBP == "MF" ) %>% mutate( diff.cor = nls.correction -  EC.gC)




diff.plot.ch.co2.gs <- SITE_DIEL_FINAL_Daily_C_same_gs %>% filter( gas == "CO2") %>% 
  ggplot(aes(x = canopyHeight_m, y= DIFF, col= RelativeDist_B)) + 
  geom_point( size = 1) + 
  geom_smooth(aes(x= canopyHeight_m, y = correction.co2 ), col="black") + 
  ylab("Difference") + xlab("Canopy Height (m)") + theme_bw() +
  theme(legend.title = element_blank())

diff.plot.ch.co2.gs
# H2O

nls_model.h2o <- nls(DIFF ~  a * exp(b * canopyHeight_m) +  exp(c * RelativeDist_B)*LAI.sd , 
                     data = SITE_DIEL_FINAL_Daily_C_same %>% filter( gas == "H2O", EC.gC < 0),
                     start = list(a = 1, b = 0.04, c = 0.01))
...
diff.plot.ch.h2o.gs <- SITE_DIEL_FINAL_Daily_C_same_gs %>% filter( gas == "H2O") %>% 
  ggplot(aes(x = canopyHeight_m, y= DIFF, col= RelativeDist_B)) + 
  geom_point( size=1) + 
  geom_smooth(aes(x= canopyHeight_m, y = correction.h2o ), col="black") + 
  ylab("Difference") + xlab("Canopy Height (m)") + theme_bw() +
  theme(legend.title = element_blank())
...
ggsave("Figures/Correction_CO2.png", plot = final.correction, width = 5, height = 2.5, units = "in")


# Adjust daily fluxes ...

SITE_DIEL_FINAL_Daily_C_adj <- SITE_DIEL_FINAL_Daily_C_same %>% 
  mutate( adj.ch  = case_when( gas == "H2O" & EC.gC <=0  ~ 4.926202 * exp(0.041578 * canopyHeight_m) +  exp(0.039910 * RelativeDist_B)*LAI.sd,
                       gas == "H2O" & EC.gC > 0  ~ 0,
                       gas == "CO2"& EC.gC <= 0 ~ 1.794971 * exp( 0.043229 * canopyHeight_m) +  exp(0.021454 * RelativeDist_B)*LAI.sd,
                       gas == "CO2" & EC.gC > 0  ~ 0),
...
plot.IGBP.fluxes <- SITE_DIEL_FINAL_Daily_C_adj_attr %>% filter(FG.gC < 50) %>% 
  ggplot( aes(x = FG.gC.adj.ch,  y= EC.gC,col=canopyHeight_m)) + 
  geom_point(size = 1, alpha=0.5) +
  stat_poly_line(col="black") +
  stat_poly_eq() + geom_abline(intercept = 0, slope = 1, col="grey") + 
  xlab(expression(paste(' Gradiet Flux (g m'^-2,  ' day'^-1,')' ))) + 
  ylab(expression(paste('Eddy Covariance (g  m'^-2,  ' day'^-1,')' ))) + theme_bw()  + 
  scale_color_gradient2(midpoint=10, low="goldenrod", mid="darkgreen", high="springgreen", name="Canopy Height (m)" )  + facet_wrap(~IGBP) +theme(
    strip.text.x = element_text(size = 10, color = "white"),
    strip.background = element_rect(fill = "black")) + xlim(-30, 30) + ylim(-30,30)
...
ggsave("Figures/Box_plot.png", plot = final.boxplot, width = 8, height = 5, units = "in")
