...
load(file.path(localdir, "SITES_One2One.Rdata"))

delta_z_site_detail_all <- SITES_One2One %>%
  mutate(delta_z = abs(MeasurementHeight_m_B - MeasurementHeight_m_A)) %>%
  filter(Canopy_L1 != "WW") %>%
  select(Site, Canopy_L1, Canopy_L2, gas, Approach, dLevelsAminusB, canopyHeight_m, delta_z, CCC, R2) %>%
  drop_na(delta_z, CCC, R2) %>%
  group_by(Site, Canopy_L1, Canopy_L2, gas, Approach) %>%
  group_modify(~ {
    d <- .x
    ...
    if (nrow(d) < 3 || dplyr::n_distinct(d$delta_z) < 2) {
      return(tibble(
        ...
        canopyHeight_m = mean(d$canopyHeight_m, na.rm = TRUE),
        CCC_mean = mean(d$CCC),
        R2_mean = mean(d$R2),
        CCC_slope = NA_real_,
        CCC_p = NA_real_,
  ...
  arrange(Canopy_L2, desc(abs_CCC_slope), desc(abs_R2_slope), Site, gas, Approach)

delta_z_site_summary_all <- delta_z_site_detail_all %>%
  filter(!is.na(abs_CCC_slope) | !is.na(abs_R2_slope)) %>%
  group_by(Canopy_L2, Site) %>%
  summarise(
    combos_tested = n(),
  ...
  arrange(Canopy_L2, desc(strongest_CCC_change), desc(strongest_R2_change), Site)

write_csv(delta_z_site_detail_all, file.path(figdir, "delta_z_all_canopy_site_trend_detail.csv"))
write_csv(delta_z_site_summary_all, file.path(figdir, "delta_z_all_canopy_site_trend_summary.csv"))
