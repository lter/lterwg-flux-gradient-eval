...
# Canopy Information:
canopy <- SITES_One2One_model %>% select( Site, rf_model, predicted, Good.CCC, CCC, dLevelsAminusB, Approach, gas, Canopy_L1) %>% rename(CCC.EC = CCC )


# Reliable Sampling Height Pairs: #### 
...
summary.val.SHP <- val.SHP %>% na.omit() %>% 
...
canopy.sub <- canopy %>% select(Site, Canopy_L1, dLevelsAminusB) %>% distinct()

val.SHP.resample <- summary.val.SHP.2 %>% 
  mutate( Approach = Approach %>% factor(levels=c("MBR", "AE","WP")) ) %>% 
  reframe(.by = c(Approach, gas, Site, dLevelsAminusB) ,
           CCC.GF = max(CCC.1.max, na.rm=T ),
  ...
  mutate(Good.CCC.EC = case_when(
         ...
         Good.CCC.GF = case_when(
               CCC.GF >= 0.95 ~ 1,.default = 0) %>% as.factor,
         predicted = case_when(
               rf_model >= 0.52 ~ 1,.default = 0) %>% as.factor,
         Good.CCC.FG.rf = case_when(
               Good.CCC.GF == 1 |  predicted == 1 ~1,
               .default = 0) %>% as.factor) %>% full_join(canopy.sub, by = c("Site", 'dLevelsAminusB'))
...
plot.approach.gf <- val.SHP.resample %>% drop_na(predicted)%>%  
  ...
  geom_hline( yintercept=0.95, col="red") + labs( x = "Approach",
                                                 y = "Maximum CCC (GF)",
                                                 colour="Prediction") 
...
plot.canopylevels.gf <-val.SHP.resample  %>% drop_na(predicted)%>%  
  ...
  geom_boxplot() + theme_bw() + scale_color_manual(values = c("0" = "skyblue", "1" = "navy")) +
  geom_hline( yintercept=0.95, col="red")+ labs( x = "Canopy Level",
                                                y = "Maximum CCC (GF)",
                                                colour="Prediction") 
...
ggsave("/Users/sm3466/YSE Dropbox/Sparkle Malone/Research/FluxGradient/lterwg-flux-gradient-eval/Figures/SamplingHeightPair_Selection_GFMODELPRED.png", plot =  final.model.plots.gf, width = 7, height = 7, units = "in")
...
canopy.adj.reliable$Good.CCC.GF
# Run-pre.validation analysis for sampling height pairs:  #####

pre.val.SHP <- validation.SamplingHeightPairs(index.df =  canopy.adj.reliable) %>% mutate(combination = paste(x, y, sep="-")) %>% rename( CCC.GF = CCC)
